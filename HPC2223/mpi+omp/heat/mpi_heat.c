...
// ricevo la prima riga della mia matrice dal rank precedente
// invio la penultima riga della mia matrice al rank successivo
   MPI_Sendrecv(&(h_T_old[NX*(NY-2)]),NX,MPI_FLOAT,next_rank,tag,&(h_T_old[NX*0]),NX,MPI_FLOAT,prev_rank,tag,MPI_COMM_WORLD,&status);

//ricevo l'ultima riga dal rank successivo
//invio la mia seconda riga al rank precedente
   MPI_Sendrecv(&(h_T_old[NX]),NX,MPI_FLOAT,prev_rank,tag,&(h_T_old[NX*(NY-1)]),NX,MPI_FLOAT,next_rank,tag,MPI_COMM_WORLD,&status);	
  ...
  if(mpi_rank == 0)   fprintf(stderr,"%f %d %d %d %d \n", t2-t1,  mpi_size, WNX, WNY, MAX_ITER);

// invio dei sottodomini a rank 0 
  MPI_Gather(&(h_T_new[NX]),NX*(NY-2),MPI_FLOAT,h_T_whole,NX*(NY-2),MPI_FLOAT,0,MPI_COMM_WORLD);
