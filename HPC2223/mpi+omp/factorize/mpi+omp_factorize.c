...
// Attiva la Ricezione della comunicazione dagli altri rank
        MPI_Irecv(&block_idx,1,MPI_INT,MPI_ANY_SOURCE,666,MPI_COMM_WORLD,&request);

        t1=MPI_Wtime();
	
	#pragma omp parallel for private(block_idx) shared(flag)
        for (block_idx=lastBlock; block_idx >= firstBlock ; block_idx--)
        {       

// Test se un e' arrivato un messaggio. Se arrivato esci 
	   ...
	   {		
			...
			{
				if(i != MPIrank)
					MPI_Send(&block_idx,1,MPI_INT,i,666,MPI_COMM_WORLD);
			}
